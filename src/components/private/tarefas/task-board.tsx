					...
					{columns.length > 0 &&
							...
							>
								...
								>
									...
									{user?.role.permissions &&
										hasPermission(user?.role.permissions!, PermissionsConstant.EDIT_LIST) &&
										editingColumnId !== column.id && (
                       ...
                       ${draggedTaskId ? "border-2 border-dashed border-indigo-primary/50" : ""}`}
								>
									...
									{/* Add Card Button */}
									{user?.role.permissions && hasPermission(user?.role.permissions!, PermissionsConstant.CREATE_JOB) && (
										<button
